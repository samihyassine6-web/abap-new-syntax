...
*&
*&---------------------------------------------------------------------*
REPORT zprg_sales_par_cursor_new.
...
LOOP AT lt_ordh INTO DATA(lwa_ordh).
  DATA(lv_index) = line_index( lt_ordi[ ono = lwa_ordh-ono ] ).
  IF lv_index IS NOT INITIAL.
    LOOP AT lt_ordi INTO DATA(lwa_ordi) FROM lv_index.                 " WHERE ono = lwa_ordh-ono.
      IF lwa_ordh-ono <> lwa_ordi-ono.  " Paralell cursor
        EXIT.
      ELSE.
